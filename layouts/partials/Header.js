...
const Header = () => {
  ...
  // We no longer have categories, so navMenu is just the static menu
  const [navMenu, setNavMenu] = useState(
    menu.main.map((item) => ({ ...item, type: "main" }))
  );

  useEffect(() => {
    // Since categories no longer exist, we always use menu.main
    ...
    <>
      <header className={`header mt-8 pt-12 pb-3`}>
        <nav className="navbar container text-center md:text-left">
          ...
          {/* Back to home when not on homepage */}
          {router.asPath !== "/" && (
                  ...
                  <li className="nav-item">
                    <Link
                      ...
                      className={`nav-link inline-block ${
                        router.asPath === menuItem.url && "nav-link-active"
                      }`}
                    >
                      {menuItem.name}
                    </Link>
